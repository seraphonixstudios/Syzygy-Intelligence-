...
| `translate` | ✅ Live + grounded | Round‑trip fidelity: source → target → back‑translate with divergence score; `degraded:true` when divergence exceeds threshold or any phase empty.  Other workflows listed below remain live without independent grounding. |
| `api_designer`, `ci_piper`, `data_pipeline`, `interview_coach`, `report_gen` | ✅ Live | Real LLM calls; task-sensitivity covered by acceptance tests, but outputs are not independently verified (no execution/grounding step) |
| `data_analyzer` | ✅ Live + grounded | Deterministic statistical analysis (rows, columns, data types, summary stats, missing‑value ratio, duplicates, z‑score outliers, Pearson correlation |r|>=0.5) runs before any LLM enhancement; `degraded:true` + `no_data_provided` when input empty; `static_analysis` flag reported. |
| `compliance` | ✅ Live + grounded | Deterministic static scan finds real line‑numbered compliance issues (hardcoded secrets, eval/exec, bare‑except, os.system, pickle, sql‑concat) before any LLM review; findings carried through the report; `degraded:true` when code empty. |
| `task_decomposition` | ✅ Live + grounded | LLM‑driven breakdown into subtasks with a verifiable JSON schema; unparseable output gets ``structured:false`` and a deterministic fallback is used. Subtask dependencies, archetypes, polarity and priority are all captured; provenance flags reported. |
